...
INTERFACE="${INTERFACE:=}"
CHANNEL="${CHANNEL:=11}"
WPA_PASSPHRASE="${WPA_PASSPHRASE:=tamtum88}"                
HW_MODE="${HW_MODE:=g}"
...
function print_banner () {
    echo -e "${MAGENTA} _____      _                 ${NC}"
    echo -e "${MAGENTA}| ____|_ __(_)_   _ _ __ ___  ${NC}"
    echo -e "${MAGENTA}|  _| | '__| | | | | '_  _ \  ${NC}"
    echo -e "${MAGENTA}| |___| |  | | |_| | | | | | |${NC}"
    echo -e "${MAGENTA}|_____|_|  |_|\__,_|_| |_| |_|${NC}"
    echo ""
}
...
function hostapd_setup {
    ...
    if grep -Fxq "erium" /etc/hostapd/hostapd.conf
        then
...
function dnsmasq_setup {
    # Create dnsmasq.conf
    if grep -Fxq "erium" /etc/dnsmasq.conf
        then
